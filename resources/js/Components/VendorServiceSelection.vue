...
import { Search, X, ChevronRight, AlertCircle, Sparkles, Utensils, Camera, Wrench } from 'lucide-vue-next'
...
const fetchServices = async () => {
    ...
    try {
        const response = await axios.get(`/api/services/${currentCategory.value.id}`, {
            params: { event_date: props.eventDate, event_pax: props.eventPax }
        })
...
// Get category icon
const getCategoryIcon = (service) => {
    if (service?.catering_service) return Utensils
    if (service?.photography_service) return Camera
    return Wrench
}
...
<template>
    <div class="h-full bg-white flex flex-col">
        ...
        <div class="sticky top-0 bg-white border-b border-gray-200 z-30 px-3 py-3 sm:px-6 sm:py-4 shadow-sm">
            <!-- Progress Bar -->
            ...
            <div class="flex items-center justify-between">
                <div class="flex-1 min-w-0">
                    <h1 v-if="currentCategory" class="text-lg sm:text-xl font-semibold text-gray-900 truncate">
                ...
                <button @click="nextStep" v-if="currentStep !== props.selectedCategories.length - 1"
                    ...
                    class="hidden sm:flex items-center bg-gray-900 text-white text-sm font-medium py-2.5 px-4 sm:px-5 rounded-lg hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-gray-800 focus:ring-offset-1 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm flex-shrink-0 ml-4">
                    Continue
        ...
        <div class="px-3 py-3 sm:px-6 sm:py-4 border-b border-gray-100 bg-gray-50">
            <div class="relative max-w-md">
                ...
                <input type="text" v-model="searchQuery"
                    class="block w-full pl-10 pr-10 py-2.5 text-sm border border-gray-300 rounded-lg placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-gray-800 focus:border-gray-800 transition-all bg-white"
                    placeholder="Search services, vendors, prices...">

                <!-- Clear search button -->
                <button v-if="searchQuery" @click="clearSearch"
                    class="absolute inset-y-0 right-0 pr-3 flex items-center">
                    <X :size="16" class="text-gray-400 hover:text-gray-600 cursor-pointer" />
                </button>
            </div>

            <!-- Search Results Info -->
            <div v-if="searchQuery && !loading" class="text-left mt-2">
                <p class="text-xs text-gray-600 bg-gray-100 inline-block px-3 py-1 rounded-full">
                    <span class="font-semibold text-gray-800">{{ filteredServices.length }}</span>
                    result{{ filteredServices.length !== 1 ? 's' : '' }} for
        ...
        <div class="flex-1 overflow-y-auto px-3 py-3 sm:px-6 sm:py-4">
            ...
            <div v-if="loading" class="flex flex-col items-center justify-center py-8 sm:py-12">
                <div class="w-8 h-8 border-2 border-gray-300 border-t-gray-800 rounded-full animate-spin mb-3"></div>
                <p class="text-gray-600 text-sm font-medium">Loading services...</p>
            </div>

            <!-- Error State -->
            <div v-else-if="errorMessage" class="mb-4 sm:mb-6 max-w-2xl mx-auto">
                    ...
                    <div class="flex items-start">
                        ...
                        <div class="ml-3 flex-1">
                            ...
                            <button @click="fetchServices"
                                class="inline-flex items-center px-3 py-1.5 bg-red-100 hover:bg-red-200 text-red-800 text-xs font-medium rounded-lg transition-colors border border-red-200">
                                Try Again
                            </button>
                        </div>
                    </div>
            ...
            <!-- Services List -->
            <div v-else-if="filteredServices.length" class="space-y-3 sm:space-y-4 max-w-4xl mx-auto">
                <div v-for="service in filteredServices" :key="service.id"
                    class="bg-white rounded-lg border transition-all duration-200 overflow-hidden hover:shadow-sm"
                    :class="{
                        'border-gray-800 border-2 ring-1 ring-gray-800 ring-opacity-10 shadow-sm': isServiceSelected(service),
                        'border-gray-200 hover:border-gray-300': !isServiceSelected(service)
                    }">

                    <!-- Selected Header Bar -->
                    <div v-if="isServiceSelected(service)"
                        class="bg-gray-900 text-white px-3 sm:px-4 py-2 text-xs font-medium">
                        ...
                        <div v-if="service.catering_service || service.photography_service">
                            <div v-if="service.catering_service">
                                <CateringSelectionCard :service="service" :isSelected="isServiceSelected(service)"
                                    ...
                                    :isPaxMinimum="service.is_pax_minimum" />
                            </div>
            ...
            <div v-else-if="searchQuery && !loading" class="text-center py-8 sm:py-12 max-w-md mx-auto">
                <div
                    class="w-12 h-12 sm:w-14 sm:h-14 mx-auto mb-3 sm:mb-4 bg-gray-100 rounded-xl flex items-center justify-center">
                    <Search :size="20" class="text-gray-400 sm:w-6 sm:h-6" />
                </div>
                ...
                <p class="text-gray-600 text-xs sm:text-sm mb-3 sm:mb-4">
                    No services match "<span class="font-medium text-gray-800">{{ searchQuery }}</span>"
                </p>
                <button @click="clearSearch"
                    class="inline-flex items-center px-4 py-2 bg-gray-900 text-white text-xs sm:text-sm font-medium rounded-lg hover:bg-gray-800 transition-colors">
                    Clear Search
                </button>
            </div>

            <!-- Empty State -->
            <div v-else-if="!loading && !searchQuery" class="text-center py-8 sm:py-12 max-w-md mx-auto">
                <div
                    class="w-12 h-12 sm:w-14 sm:h-14 mx-auto mb-3 sm:mb-4 bg-gray-100 rounded-xl flex items-center justify-center">
                    <component :is="getCategoryIcon(services[0])" :size="20" class="text-gray-400 sm:w-6 sm:h-6" />
                </div>
                <h3 class="text-sm sm:text-base font-semibold text-gray-900 mb-1">No services available</h3>
                <p class="text-gray-600 text-xs sm:text-sm">We couldn't find any services for this category.</p>
            </div>
        </div>
        ...
        <div v-if="currentStep !== props.selectedCategories.length - 1"
            ...
            <button @click="nextStep"
                :disabled="!selectedServices.some(s => s.category?.id === currentCategory.id || s.category_id === currentCategory.id)"
                class="w-full bg-gray-900 text-white text-sm font-medium py-3 px-6 rounded-lg hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-gray-800 focus:ring-offset-1 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center min-h-[48px]">
                Continue
                <ChevronRight :size="16" class="ml-2" />
            </button>
        </div>
