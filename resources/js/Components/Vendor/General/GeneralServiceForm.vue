...
// ✅ Watch for category changes (reactive + handles mount)
watch(
...
const handleImageUpload = (event) => {
    ...
    files.forEach(file => {
        if (file && file.type.startsWith('image/')) {
            const existsIndex = selectedImages.value.findIndex(
...
const submit = () => {
    form.post(route('vendor.services.store'), {
        preserveScroll: true,
        onSuccess: () => {
            // toast.success(`${props.selectedCategory} service created successfully`);
            push.success(`${props.selectedCategory} service created successfully`);
            form.reset();
        ...
        onError: () => {
            // toast.error(`Failed to create ${props.selectedCategory} service`);
            push.error(`Failed to create ${props.selectedCategory} service`);
        }
    });
};
</script>
