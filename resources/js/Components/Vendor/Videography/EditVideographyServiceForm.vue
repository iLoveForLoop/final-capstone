...
import {
    Upload, X, Plus, Image as ImageIcon, Eye, Video,
    Tag, FileText, CheckCircle, Settings, Circle,
    Loader, Edit3, Clock, Users, Calendar
...
const form = useForm({
    ...
    cover_images: [],
    notes: props.service.videography_service?.notes || '',

    // Videography specific fields
    studio_shoot_available: props.service.videography_service?.studio_shoot_available || false,
    // hours_of_coverage: props.service.videography_service?.hours_of_coverage || 8,
    // delivery_time_days: props.service.videography_service?.delivery_time_days || 30,
    // number_of_videographers: props.service.videography_service?.number_of_videographers || 1,
    specifications: props.service.videography_service?.specifications || [],
...
const submit = () => {
    form.post(route('vendor.videography-services.update', props.service.id), {
        preserveScroll: true,
        onSuccess: () => {
            push.success('Videography service updated successfully');
            emit('close');
            emit('updated');
        },
        onError: (errors) => {
            push.error('Failed to update videography service');
        }
    });
};
</script>
        ...
        <div class="space-y-8">
            ...
            <div class="border-b border-gray-200 pb-8">
                <div class="flex items-center gap-3 mb-6">
                    ...
                    <h4 class="text-lg font-semibold text-gray-900">Videography Details</h4>
                </div>
