...
const form = useForm({
    ...
    price: props.cateringService.price || '',
    max_price: props.service.max_price || '',
    cover_images: [],
    min_pax: props.cateringService.min_pax || 50,
    max_pax: props.cateringService.max_pax || 100,
    package_price: props.cateringService.package_price || '',
    service_area: props.cateringService.service_area || [],
    is_customizable: props.cateringService.is_customizable || false,
    delivery_fee: props.cateringService.delivery_fee || '',
    specifications: props.service.specifications || [],
    dishes: props.cateringService.dishes || {},
    dish_selection_limits: props.cateringService.dish_selection_limits || {},
    notes: props.cateringService.notes || '',
    deleted_images: [],
    _method: 'PUT'
});
...
const handleImageUpload = (event) => {
    ...
    const totalImages = selectedImages.value.length + existingImages.value.length - imagesToDelete.value.length;

    files.forEach(file => {
        if (file && file.type.startsWith('image/') && totalImages < 5) {
...
const removeExistingImage = (imageId) => {
    ...
    if (imageIndex !== -1) {
        const image = existingImages.value[imageIndex];
        imagesToDelete.value.push(imageId);
        existingImages.value.splice(imageIndex, 1);
        form.deleted_images = imagesToDelete.value;
    }
};

const restoreImage = (imageId) => {
    const deleteIndex = imagesToDelete.value.findIndex(id => id === imageId);
    if (deleteIndex !== -1) {
...
const submit = () => {
    form.post(route('vendor.catering-services.update', props.service.id), {
        preserveScroll: true,
        onSuccess: () => {
            // toast.success('Service updated successfully');
            push.success('Service updated successfully')
            emit('close');
            emit('updated');
        },
        onError: () => {
            // toast.error('Failed to update service');
            push.error('Failed to update service');
        }
    });
};
...
const getTotalImageCount = () => {
    return selectedImages.value.length + existingImages.value.length;
};
</script>
        ...
        <div class="space-y-8">
            ...
            <div class="border-b border-gray-200 pb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                                ...
                                <div v-for="image in existingImages" :key="image.id"
                                    ...
                                    <div
                                        ...
                                        <div
                                            ...
                                            <button type="button"
                                                @click="imagesToDelete.includes(image.id) ? restoreImage(image.id) : removeExistingImage(image.id)"
                                                class="p-2 bg-white rounded-lg transition-colors"
                                                :class="imagesToDelete.includes(image.id) ? 'text-green-600 hover:text-green-700' : 'text-red-600 hover:text-red-700'">
                                                <X v-if="!imagesToDelete.includes(image.id)" class="w-4 h-4" />
                                    ...
                                    <div v-if="imagesToDelete.includes(image.id)"
                                        ...
                                        <span class="text-red-700 text-sm font-medium">Will be deleted</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Image Upload Area -->
                        <div class="mb-6" v-if="getTotalImageCount() < 5">
                            <label class="cursor-pointer">
                                <div
                                    ...
                                    <p class="text-xs text-gray-500 mt-2">
                                        PNG, JPG, JPEG up to 2MB each (Max 5 images total)
                                    </p>
                                </div>
                                <input type="file" class="sr-only" @change="handleImageUpload" accept="image/*" multiple
                                    :disabled="getTotalImageCount() >= 5">
                            </label>
                        </div>

                        <!-- New Selected Images Grid -->
                        <div v-if="selectedImages.length > 0">
                            <h5 class="text-sm font-medium text-gray-600 mb-3">New Images to Add</h5>
                            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                                <div v-for="(image, index) in selectedImages" :key="image.id"
                                    class="relative group rounded-lg overflow-hidden bg-gray-100 aspect-square">
                                    <img :src="image.preview" :alt="`New service image ${index + 1}`"
                        ...
                        <div class="mt-4 flex items-center justify-between text-sm text-gray-500">
                            <span>{{ getTotalImageCount() }} of 5 images</span>
                            <span v-if="getTotalImageCount() > 0" class="text-xs">
                                First remaining image will be used as the primary cover
                            </span>
                        </div>
            ...
            <div>
                ...
                <textarea id="notes" v-model="form.notes" rows="4"
                    ...
                    placeholder="Any additional information about your catering services..."></textarea>
