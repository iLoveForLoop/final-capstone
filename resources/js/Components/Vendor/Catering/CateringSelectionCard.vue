...
const closeDishModal = () => {
    dishModal.value?.closeModal();
};

const toggleDishModal = () => {
    dishModal.value?.toggleModal();
};
...
<template>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-5 space-y-5 hover:shadow-sm transition-shadow">
        ...
        <div v-show="showDetails" class="space-y-4 pt-2">
            ...
            <!-- Show selected dishes if any -->
            <div v-if="hasSelections()" class="space-y-2">
                <h4 class="text-xs font-semibold text-gray-700 flex items-center">
                    ...
                    Your Selections
                    <span class="text-xs bg-green-100 text-green-700 px-1.5 py-0.5 rounded-full ml-1.5">
                        {{ getSelectedDishCount() }} selected
                    </span>
                </h4>
        ...
        <div v-if="isDateAvailable" class="flex gap-3 pt-4 border-t border-gray-100">
            ...
            <button @click="handleSelect" :class="{
                'bg-[#239BA7] text-white hover:bg-[#1D8E99]': !hasSelections(),
