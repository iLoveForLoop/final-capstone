...
import { Eye, X, Image as ImageIcon, Users, Utensils, MapPin, CheckCircle, Star, Calendar } from 'lucide-vue-next';
...
// Parse dishes data safely
const menuCategories = computed(() => {
    if (!props.cateringService.dishes) return [];

    try {
        if (typeof props.cateringService.dishes === 'string') {
            return Object.entries(JSON.parse(props.cateringService.dishes));
        }
        return Object.entries(props.cateringService.dishes);
    } catch (e) {
...
// Parse dish selection limits safely
const dishSelectionLimits = computed(() => {
    if (!props.cateringService.dish_selection_limits) return {};

    try {
        if (typeof props.cateringService.dish_selection_limits === 'string') {
            return JSON.parse(props.cateringService.dish_selection_limits);
        }
        return props.cateringService.dish_selection_limits;
    } catch (e) {
        console.error('Error parsing dish selection limits:', e);
        return {};
    }
});
...
const serviceAreas = computed(() => {
    if (!props.cateringService.service_area) return [];

    try {
        if (typeof props.cateringService.service_area === 'string') {
            return JSON.parse(props.cateringService.service_area);
        }
        return props.cateringService.service_area;
    } catch (e) {
...
const specifications = computed(() => {
    if (!props.cateringService.specifications) return [];

    try {
        if (typeof props.cateringService.specifications === 'string') {
            return JSON.parse(props.cateringService.specifications);
        }
        return props.cateringService.specifications;
    } catch (e) {
                    ...
                    <div class="bg-white px-6 py-4 border-b border-gray-200 sticky top-0 bg-white z-10">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-sm text-gray-500 mt-1">Complete overview of your catering service</p>
                            </div>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <div class="lg:col-span-2 space-y-6">
                                ...
                                <div v-if="portfolioImages.length > 0">
                                    <h4 class="text-lg font-semibold text-gray-900 mb-4">Service Images</h4>
                                ...
                                <div v-if="menuCategories.length > 0"
                                    ...
                                    <h4 class="text-lg font-semibold text-gray-900 mb-4">Menu Details</h4>
                                    <div class="space-y-4">
                                        <div v-for="[category, dishes] in menuCategories" :key="category"
                                            ...
                                            <div class="flex items-center justify-between mb-3">
                                                <h5 class="text-md font-semibold text-gray-800 flex items-center">
                                                    <Utensils class="w-4 h-4 mr-2 text-green-600" />
                                                    {{ category }}
                                                </h5>
                                                <div v-if="cateringService.is_customizable"
                                                    class="flex items-center space-x-2">
                                                    <span class="text-sm text-gray-600">Select up to:</span>
                                                    <span
                                                        class="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-sm font-medium">
                                                        {{ dishSelectionLimits[category] || 1 }}
                                                    </span>
                                                </div>
                                            </div>
                            ...
                            <div class="space-y-6">
                                ...
                                <div class="bg-white border border-gray-200 rounded-xl p-6">
                                    ...
                                    <div class="space-y-4">
                                        ...
                                        <!-- Per Pax Pricing -->
                                        <div v-if="cateringService.price" class="space-y-2">
                                            <div class="flex items-baseline justify-between">
                                                <span class="text-sm text-gray-600">Price per Pax</span>
                                                <span class="text-xl font-bold text-indigo-600">{{
                                                    formatPrice(cateringService.price) }}</span>
                                            </div>
                                        </div>

                                        <!-- Package Pricing -->
                                        <div v-if="cateringService.package_price" class="space-y-2">
                                            <div class="flex items-baseline justify-between">
                                                <span class="text-sm text-gray-600">Package Price</span>
                                                <span class="text-xl font-bold text-indigo-600">{{
                                                    formatPrice(cateringService.package_price) }}</span>
                                            </div>
                                        </div>
                                        ...
                                        <div class="pt-3 border-t border-gray-200">
                                            <div class="flex items-center justify-between">
                                                <span class="text-sm text-gray-600">Capacity Range</span>
                                                <span class="font-semibold text-gray-900">
                                                    {{ cateringService.min_pax || 50 }} - {{ cateringService.max_pax ||
                                                        100
                                                    }} pax
                                                </span>
                                            </div>
                                ...
                                <!-- Service Options -->
                                <div class="bg-white border border-gray-200 rounded-xl p-6">
                                    <h4 class="text-lg font-semibold text-gray-900 mb-4">Service Options</h4>
                                    <div class="space-y-4">
                                        <!-- Customizable Menu -->
                                        <div class="flex items-center justify-between">
                                            <span class="text-sm text-gray-600">Customizable Menu</span>
                                            <div class="flex items-center space-x-1">
                                                <CheckCircle class="w-4 h-4"
                                                    :class="cateringService.is_customizable ? 'text-green-500' : 'text-gray-400'" />
                                                <span class="font-medium"
                                                    :class="cateringService.is_customizable ? 'text-green-700' : 'text-gray-500'">
                                                    {{ cateringService.is_customizable ? 'Enabled' : 'Disabled' }}
                                                </span>
                                            </div>
                                        </div>
                                        ...
                                        <!-- Service Areas -->
                                        <div>
                                            <span class="text-sm text-gray-600 block mb-2">Service Areas:</span>
                                            <div class="flex flex-wrap gap-2">
                                                <span v-for="(area, index) in serviceAreas" :key="index"
                                                    class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                                    <MapPin class="w-3 h-3 mr-1" />
                                                    {{ area }}
                                                </span>
                                                <span v-if="serviceAreas.length === 0" class="text-xs text-gray-500">
                                        ...
                                        <div v-if="specifications.length > 0">
                                            ...
                                            <div class="flex flex-wrap gap-2">
                                                <span v-for="(spec, index) in specifications" :key="index"
