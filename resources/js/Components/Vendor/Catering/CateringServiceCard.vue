        ...
        <div class="p-4">
            ...
            <div class="grid grid-cols-2 gap-3 mb-4">
                ...
                <div class="flex items-center text-sm text-gray-700">
                    ...
                    <span>{{ service.catering_service.min_pax }}-{{ service.catering_service.max_pax }} pax</span>
                </div>
                ...
                <div class="flex items-center text-sm text-gray-700">
                    ...
                    <span>{{ service.catering_service.lead_time_days }} days</span>
                </div>
                ...
                <div class="flex items-center text-sm text-gray-700">
                    ...
                    <span>{{ formatPrice(service.catering_service.price) }}</span>
                </div>

                <!-- Delivery -->
                <div class="flex items-center text-sm text-gray-700" v-if="service.catering_service.delivery_fee">
                    <svg class="h-4 w-4 mr-2 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path d="M9 17a2 2 0 11-4 0 2 2 0 014 0zM19 17a2 2 0 11-4 0 2 2 0 014 0z" />
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10a1 1 0 001 1h1m8-1a1 1 0 01-1 1H9m4-1V8a1 1 0 011-1h2.586a1 1 0 01.707.293l3.414 3.414a1 1 0 01.293.707V16a1 1 0 01-1 1h-1m-6-1a1 1 0 001 1h1M5 17a2 2 0 104 0m-4 0a2 2 0 114 0m6 0a2 2 0 104 0m-4 0a2 2 0 114 0" />
                    </svg>
                    <span>{{ formatPrice(service.catering_service.delivery_fee) }}</span>
                </div>
            </div>
            ...
            <!-- Dishes Preview -->
            <div v-if="service.catering_service.dishes && service.catering_service.dishes.length" class="mb-3">
                <h4 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Menu Includes</h4>
                <div class="flex flex-wrap gap-1">
                    <span v-for="(dish, index) in service.catering_service.dishes.slice(0, 5)" :key="index"
                        class="text-xs px-2 py-1 bg-green-50 text-green-700 rounded-full">
                        {{ dish }}
                    </span>
                    <span v-if="service.catering_service.dishes.length > 5"
                        class="text-xs px-2 py-1 bg-gray-50 text-gray-500 rounded-full">
                        +{{ service.catering_service.dishes.length - 5 }} more
                    </span>
                </div>
            </div>

            <!-- Customizable Badge -->
            <div v-if="service.catering_service.is_customizable" class="mb-3">
                <span class="text-xs px-2 py-1 bg-purple-50 text-purple-700 rounded-full font-medium">
                    ✨ Customizable Menu
                </span>
            </div>
